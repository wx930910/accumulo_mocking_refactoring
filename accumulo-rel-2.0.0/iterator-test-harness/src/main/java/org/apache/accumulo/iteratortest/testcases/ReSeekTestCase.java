...
public class ReSeekTestCase extends OutputVerifyingTestCase {
	...
	/**
	 * Let N be a random number between [0, RESEEK_INTERVAL). After every Nth
	 * entry "returned" to the client, recreate and reseek the iterator.
	 */
	...
	TreeMap<Key, Value> consume(SortedKeyValueIterator<Key, Value> skvi, IteratorTestInput testInput)
		...
		while (skvi.hasTop()) {
			...
			/*
			 * One of the trickiest cases in writing iterators: After any result
			 * is returned from a TabletServer to the client, the Iterator in
			 * the TabletServer's memory may be torn down. To preserve the state
			 * and guarantee that all records are received, the TabletServer
			 * does remember the last Key it returned to the client. It will
			 * recreate the Iterator (stack), and seek it using an updated
			 * Range. This range's start key is set to the last Key returned,
			 * non-inclusive.
			 */
			if (i % RESEEK_INTERVAL == reseekCount) {
				...
				// IteratorEnvironment ie = Mockito.
				skvi.init(sourceCopy, testInput.getIteratorOptions(), new SimpleIteratorEnvironment());

				// The new range, resume where we left off (non-inclusive), with
				// same families filter
