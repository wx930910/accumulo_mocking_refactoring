...
public class TracerTest {
	...
	static class TestReceiver implements SpanReceiver {
		...
		public void receiveSpan(Span s) {
			...
			if (!traces.containsKey(traceId)) traces.put(traceId, new ArrayList<>());
	...
	public void testThrift() throws Exception {
		...
		TestService.Iface se = Mockito.mock(TestService.Iface.class);
		Mockito.when(se.checkTrace(Mockito.any(), Mockito.anyString())).thenAnswer(invo -> {
			String message = invo.getArgument(1);
			try (TraceScope trace = Trace.startSpan(message)) {
				return Trace.isTracing();
			}
		});
		args.processor(new Processor<Iface>(TraceUtil.wrapService(se)));
	 ...
	 * sampling enabled, as the cause of the reflexive exceptions.
	 */
	...
	/**
	 * Verify that exceptions propagate up through the trace wrapping with
	 * sampling disabled, as the cause of the reflexive exceptions.
	 */
	@Test(expected = IOException.class)
