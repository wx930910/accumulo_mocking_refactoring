...
public abstract class BaseHostRegexTableLoadBalancerTest extends HostRegexTableLoadBalancer {
	...
	protected boolean tabletInBounds(KeyExtent ke, TServerInstance tsi) {
		...
		if (tid.equals("1") && (host.equals("192.168.0.1") || host.equals("192.168.0.2") || host.equals("192.168.0.3")
				|| host.equals("192.168.0.4") || host.equals("192.168.0.5"))) {
			return true;
		} else if (tid.equals("2") && (host.equals("192.168.0.6") || host.equals("192.168.0.7")
				|| host.equals("192.168.0.8") || host.equals("192.168.0.9") || host.equals("192.168.0.10"))) {
					return true;
				} else
			return tid.equals("3") && (host.equals("192.168.0.11") || host.equals("192.168.0.12")
					|| host.equals("192.168.0.13") || host.equals("192.168.0.14") || host.equals("192.168.0.15"));
	}
	...
	protected TabletBalancer getBalancerForTable(TableId table) {
		TabletBalancer res = Mockito.mock(TabletBalancer.class);
		try {
			Mockito.doAnswer(invo -> {
				TServerInstance tserver = invo.getArgument(0);
				TableId tableId = invo.getArgument(1);
				String tableName = idToTableName(tableId);
				TServerInstance initialLocation = initialTableLocation.get(tableName);
				if (tserver.equals(initialLocation)) {
					List<TabletStats> list = new ArrayList<>(5);
				...
				return null;
			}).when(res).getOnlineTabletsForTable(Mockito.any(), Mockito.any());
