...
public class TableLoadBalancerTest {
	...
	// ugh... so wish I had provided mock objects to the LoadBalancer in the
	// master
	...
	public void test() {
		...
		ServerConfigurationFactory confFactory = new ServerConfigurationFactory(context, new SiteConfiguration()) {
			...
			public TableConfiguration getTableConfiguration(TableId tableId) {
				// create a dummy namespaceConfiguration to satisfy
				// requireNonNull in TableConfiguration
				// constructor
				...
				return new TableConfiguration(context, tableId, dummyConf) {
					...
					public String get(Property property) {
						// fake the get table configuration so the test doesn't
						// try to look in zookeeper for
		...
		org.apache.accumulo.server.master.balancer.TableLoadBalancer tls = Mockito
				.spy(org.apache.accumulo.server.master.balancer.TableLoadBalancer.class);
		Mockito.doAnswer(invo -> DefaultLoadBalancer.class.getName()).when(tls)
				.getLoadBalancerClassNameForTable(Mockito.any());
		try {
			Mockito.doAnswer(invo -> generateFakeTablets(invo.getArgument(0), invo.getArgument(1))).when(tls)
					.getOnlineTabletsForTable(Mockito.any(), Mockito.any());
			Mockito.doAnswer(invo -> {
				TableOperations tops = EasyMock.createMock(TableOperations.class);
				EasyMock.expect(tops.tableIdMap()).andReturn(TABLE_ID_MAP).anyTimes();
				replay(tops);
				return tops;
			}).when(tls).getTableOperations();
