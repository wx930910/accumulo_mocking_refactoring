...
public class PerTableVolumeChooserTest {
	...
	private VolumeChooser getTableDelegate() {
		VolumeChooserEnvironment env = new VolumeChooserEnvironmentImpl(TableId.of("testTable"), null, null) {
			@Override
	...
	public void testTableScopeUsingTableProperty() throws Exception {
		...
		expect(serviceEnv.instantiate(TableId.of("testTable"), MockChooser1.class.getName(), VolumeChooser.class))
	...
	public void testTableScopeUsingDefaultScopeProperty() throws Exception {
		...
		expect(serviceEnv.instantiate(TableId.of("testTable"), MockChooser2.class.getName(), VolumeChooser.class))
	...
	public void testTableScopeWithNoConfig() {
		expect(tableConf.getTableCustom(TABLE_CUSTOM_SUFFIX)).andReturn(null).once();
		expect(systemConf.getCustom(getCustomPropertySuffix(ChooserScope.DEFAULT))).andReturn(null).once();
		replay(serviceEnv, tableConf, systemConf);
	...
	public void testTableScopeWithBadDelegate() throws Exception {
		expect(tableConf.getTableCustom(TABLE_CUSTOM_SUFFIX)).andReturn(null).once();
		expect(systemConf.getCustom(getCustomPropertySuffix(ChooserScope.DEFAULT))).andReturn("not a valid class name")
				.once();
		expect(serviceEnv.instantiate(TableId.of("testTable"), "not a valid class name", VolumeChooser.class))
	...
	public void testLoggerScopeUsingLoggerProperty() throws Exception {
		...
		expect(serviceEnv.instantiate(MockChooser1.class.getName(), VolumeChooser.class)).andReturn(new MockChooser1());
		replay(serviceEnv, tableConf, systemConf);
	...
	public void testLoggerScopeUsingDefaultProperty() throws Exception {
		expect(systemConf.getCustom(getCustomPropertySuffix(ChooserScope.LOGGER))).andReturn(null).once();
		expect(systemConf.getCustom(getCustomPropertySuffix(ChooserScope.DEFAULT)))
				.andReturn(MockChooser2.class.getName()).once();
		expect(serviceEnv.instantiate(MockChooser2.class.getName(), VolumeChooser.class)).andReturn(new MockChooser2());
		replay(serviceEnv, tableConf, systemConf);
	...
	public void testLoggerScopeWithBadDelegate() throws Exception {
		...
		expect(systemConf.getCustom(getCustomPropertySuffix(ChooserScope.DEFAULT))).andReturn("not a valid class name")
				.once();
		expect(serviceEnv.instantiate("not a valid class name", VolumeChooser.class)).andThrow(new RuntimeException());
		replay(serviceEnv, tableConf, systemConf);
