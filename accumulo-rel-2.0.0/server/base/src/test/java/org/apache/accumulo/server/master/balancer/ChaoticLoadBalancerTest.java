...
public class ChaoticLoadBalancerTest {
	...
	class FakeTServer {
		...
		TabletServerStatus getStatus() {
			...
			for (KeyExtent extent : extents) {
				...
				if (info == null) result.tableMap.put(table.canonical(), info = new TableInfo());
				info.onlineTablets++;
	...
	public void testAssignMigrations() {
		...
		ChaoticLoadBalancer balancer = Mockito.spy(ChaoticLoadBalancer.class);
		try {
			Mockito.doAnswer(invo -> {
				TServerInstance tserver = invo.getArgument(0);
				TableId tb = invo.getArgument(1);
				List<TabletStats> result = new ArrayList<>();
				for (KeyExtent extent : servers.get(tserver).extents) {
					if (extent.getTableId().equals(tb)) {
						result.add(new TabletStats(extent.toThrift(), null, null, null, 0L, 0., 0., 0));
					}
				}
				return result;
			}).when(balancer).getOnlineTabletsForTable(Mockito.any(), Mockito.any());
		} catch (TException e) {
			e.printStackTrace();
		}
	...
	public void testUnevenAssignment() {
		...
		// Put more tablets on one server, but not more than the number of
		// servers
		...
		// Just want to make sure it gets some migrations, randomness prevents
		// guarantee of a defined
