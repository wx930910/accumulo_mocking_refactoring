...
public class DefaultLoadBalancerTest {
	...
	class FakeTServer {
		...
		TabletServerStatus getStatus() {
			...
			for (KeyExtent extent : extents) {
				...
				if (info == null) result.tableMap.put(tableId.canonical(), info = new TableInfo());
				info.onlineTablets++;
	...
	public void testAssignMigrations() {
		...
		DefaultLoadBalancer balancer = Mockito.spy(DefaultLoadBalancer.class);
		try {
	...
	public void testUnevenAssignment2() {
		...
		// average is 58, with 2 at 59: we need 48 more moved to the short
		// server
