...
public class RootTabletStateStoreTest {
	...
	private DistributedStore mockDistributedStore() {
		DistributedStore res = Mockito.mock(DistributedStore.class);
		Node root = new Node("/");
		try {
			Mockito.when(res.getChildren(Mockito.anyString())).thenAnswer(invo -> {
				String path = invo.getArgument(0);
				Node node = navigate(root, path);
			...
			Mockito.doAnswer(invo -> {
				String path = invo.getArgument(0);
				byte[] bs = invo.getArgument(1);
				create(root, path).value = bs;
				return null;
			}).when(res).put(Mockito.anyString(), Mockito.any());
			Mockito.doAnswer(invo -> {
				String path = invo.getArgument(0);
				...
				return null;
			}).when(res).remove(Mockito.anyString());
			Mockito.when(res.get(Mockito.anyString())).thenAnswer(invo -> {
				String path = invo.getArgument(0);
				Node node = navigate(root, path);
	...
	private Node navigate(Node root, String path) {
	...
	private Node create(Node root, String path) {
		String[] parts = path.split("/");
	...
	public void testFakeZoo() throws DistributedStoreException {
		DistributedStore store = mockDistributedStore();
	...
	public void testRootTabletStateStore() throws DistributedStoreException {
		ZooTabletStateStore tstore = new ZooTabletStateStore(mockDistributedStore());
