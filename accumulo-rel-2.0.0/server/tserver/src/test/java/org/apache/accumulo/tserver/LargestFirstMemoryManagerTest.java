...
public class LargestFirstMemoryManagerTest {
	...
	private TabletState mockTabletState(KeyExtent extent, long commit, long memsize, long compactingTableSize) {
		TabletState res = Mockito.mock(TabletState.class);
		Mockito.when(res.getExtent()).thenReturn(extent);
		Mockito.when(res.getLastCommitTime()).thenReturn(commit);
		Mockito.when(res.getMemTableSize()).thenReturn(memsize);
		Mockito.when(res.getMinorCompactingMemTableSize()).thenReturn(compactingTableSize);
		return res;
	}

	private TabletState t(KeyExtent ke, long lastCommit, long memSize, long compactingSize) {
		return mockTabletState(ke, lastCommit, memSize, compactingSize);
	}
