...
public class InMemoryMapTest {
	...
	private IteratorEnvironment mockIteratorEnvironment() {
		final SamplerConfiguration sampleConfig = null;
		IteratorEnvironment ie = Mockito.mock(IteratorEnvironment.class);
		Mockito.when(ie.isSamplingEnabled()).thenAnswer(invo -> sampleConfig != null);
		Mockito.when(ie.getSamplerConfiguration()).thenAnswer(invo -> sampleConfig);
		return ie;
	}
	...
	public void test6() throws Exception {
		...
		SortedKeyValueIterator<Key, Value> dc = ski1.deepCopy(mockIteratorEnvironment());
	...
	private void deepCopyAndDelete(int interleaving, boolean interrupt) throws Exception {
		// interleaving == 0 intentionally omitted, this runs the test w/o
		// deleting in mem map
