...
public class LargeRowFilterTest {
	...
	private LargeRowFilter setupIterator(TreeMap<Key, Value> testData, int maxColumns, IteratorScope scope)
			throws IOException {
		...
		boolean fmc = false;
		Mockito.when(ie.getIteratorScope()).thenReturn(scope);
		Mockito.when(ie.isFullMajorCompaction()).thenReturn(fmc);
		lrfi.init(new ColumnFamilySkippingIterator(smi), is.getOptions(), ie);
		return lrfi;
	}
	...
	public void testSeek() throws Exception {
		...
		for (int i = 1; i <= 20; i++) {
			...
			// seek to each row... rows that exceed max columns should be
			// filtered
			for (int j = 1; j <= i; j++) {
	...
	public void testCompaction() throws Exception {
		...
		// try seeking to the middle of row 15... row has data and suppression
		// marker... this seeks past
