...
public class AccumuloConfigurationTestMock {
	@Rule
	...
	private void set(String p, String v, HashMap<String, String> props, long[] upCount) {
		props.put(p, v);
		upCount[0]++;
	}
	...
	public void testGetByPrefix() {
		// This test checks that when anything changes that all prefix maps are
		// regenerated. However
		// when there are not changes the test expects all the exact same
		// map to always be returned.
		AccumuloConfiguration tc = Mockito.spy(AccumuloConfiguration.class);
		HashMap<String, String> props = new HashMap<>();
		long[] upCount = new long[1];
		AccumuloConfiguration parent = null;
		Mockito.doAnswer(invo -> {
			Property prop = invo.getArgument(0);
			return props.containsKey(prop.getKey());
		}).when(tc).isPropertySet(Mockito.any(Property.class), Mockito.anyBoolean());
		Mockito.when(tc.getUpdateCount()).thenReturn(upCount[0]);
		Mockito.when(tc.get(Mockito.any(Property.class))).thenAnswer(invo -> {
			String v = props.get((Property) invo.getArgument(0));
			if (v == null & parent != null) {
				v = parent.get((Property) invo.getArgument(0));
			}
			return v;
		});
		Mockito.doAnswer(invo -> {
			Map<String, String> output = invo.getArgument(0);
			Predicate<String> filter = invo.getArgument(1);
			if (parent != null) {
				parent.getProperties(output, filter);
			}
			for (Entry<String, String> entry : props.entrySet()) {
			...
			return null;
		}).when(tc).getProperties(Mockito.anyMap(), Mockito.any());

		set(Property.TABLE_ARBITRARY_PROP_PREFIX.getKey() + "a1", "325", props, upCount);
		set(Property.TABLE_ARBITRARY_PROP_PREFIX.getKey() + "a2", "asg34", props, upCount);

		set(Property.TABLE_ITERATOR_SCAN_PREFIX.getKey() + "i1", "class34", props, upCount);
		set(Property.TABLE_ITERATOR_SCAN_PREFIX.getKey() + "i1.opt", "o99", props, upCount);
		...
		// ensure getting one prefix does not cause others to unnecessarily
		// regenerate
		...
		set(Property.TABLE_ITERATOR_SCAN_PREFIX.getKey() + "i2", "class42", props, upCount);
		set(Property.TABLE_ITERATOR_SCAN_PREFIX.getKey() + "i2.opt", "o78234", props, upCount);
		...
		set(Property.VFS_CONTEXT_CLASSPATH_PROPERTY.getKey() + "ctx123", "hdfs://ib/p1", props, upCount);
	...
	public void testScanExecutors() {
		...
		ScanExecutorConfig sec8 = tc.getScanExecutors().stream().filter(c -> c.name.equals("hulksmash")).findFirst()
				.get();
