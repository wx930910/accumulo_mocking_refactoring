...
public class RateLimitedInputStreamTest {
	...
	public void permitsAreProperlyAcquired() throws Exception {
		...
		// Mock RateLimiter
		org.apache.accumulo.core.util.ratelimit.RateLimiter limiter = Mockito
				.mock(org.apache.accumulo.core.util.ratelimit.RateLimiter.class);
		// Stub methods, replace field modification to local variable
		// modification
		Mockito.when(limiter.getRate()).thenReturn((long) 0);
		Mockito.doAnswer(invo -> {
			long permits = invo.getArgument(0);
			permitsAcquired.addAndGet(permits);
			return null;
		}).when(limiter).acquire(Mockito.anyLong());
		long bytesRetrieved = 0;
		try (InputStream is = new RateLimitedInputStream(new RandomInputStream(), limiter)) {
			for (int i = 0; i < 100; ++i) {
		...
		// Replace getter methods by directly using local variable.
		assertEquals(bytesRetrieved, permitsAcquired.get());
	}
