...
import org.apache.accumulo.core.util.Merge.MergeException;
...
public class MergeTest {
	...
	static class MergeTester extends Merge {
		...
		@Override
		protected Iterator<Size> getSizeIterator(AccumuloClient client, String tablename, final Text start,
	...
	private static Merge mockMerge(List<List<Size>> merges, Integer... sizes) {
		Merge res = Mockito.spy(Merge.class);
		List<Size> tablets = new ArrayList<>();
		Text start = null;
		for (Integer size : sizes) {
			Text end;
			if (tablets.size() == sizes.length - 1) end = null;
		...
		Mockito.doNothing().when(res).message(Mockito.anyString(), Mockito.any());
		try {
			Mockito.doAnswer(invo -> {
				AccumuloClient client = invo.getArgument(0);
				String tablename = invo.getArgument(1);
				final Text s = invo.getArgument(2);
				final Text end = invo.getArgument(3);
				final Iterator<Size> impl = tablets.iterator();
				return new Iterator<Size>() {
					Size next = skip();
					...
					private Size skip() {
						while (impl.hasNext()) {
							...
							if (s != null) {
								if (candidate.extent.getEndRow() != null
										&& candidate.extent.getEndRow().compareTo(s) < 0)
									continue;
							}
							if (end != null) {
								if (candidate.extent.getPrevEndRow() != null
			...
			}).when(res).getSizeIterator(Mockito.any(), Mockito.anyString(), Mockito.any(), Mockito.any());
			Mockito.doAnswer(invo -> {
				List<Size> s = invo.getArgument(2);
				int numToMerge = invo.getArgument(3);
				List<Size> merge = new ArrayList<>();
				for (int i = 0; i < numToMerge; i++) {
					merge.add(s.get(i));
				}
				merges.add(merge);
				return null;
			}).when(res).merge(Mockito.any(), Mockito.anyString(), Mockito.anyList(), Mockito.anyInt());
	...
	public void testMergomatic() throws Exception {
		...
		List<List<Size>> merges = new ArrayList<>();
		int i;
		Merge test = mockMerge(merges, 10, 20, 30);
		test.mergomatic(null, "table", null, null, 1000, false);
		assertEquals(1, merges.size());
		assertArrayEquals(new int[] { 10, 20, 30 }, sizes(merges.get(i = 0)));
		merges = new ArrayList<>();
		// Merge ranges around tablets that are big enough
		test = mockMerge(merges, 1, 2, 100, 1000, 17, 1000, 4, 5, 6, 900);
		test.mergomatic(null, "table", null, null, 1000, false);
		assertEquals(2, merges.size());
		assertArrayEquals(new int[] { 1, 2, 100 }, sizes(merges.get(i = 0)));
		assertArrayEquals(new int[] { 4, 5, 6, 900 }, sizes(merges.get(++i)));
		merges = new ArrayList<>();
		...
		assertArrayEquals(new int[] { 1, 2, 100 }, sizes(merges.get(i = 0)));
		assertArrayEquals(new int[] { 17, 1000 }, sizes(merges.get(++i)));
		assertArrayEquals(new int[] { 4, 5, 6, 900 }, sizes(merges.get(++i)));
		merges = new ArrayList<>();
		// Limit the low-end of the merges
		test = mockMerge(merges, 1, 2, 1000, 17, 1000, 4, 5, 6, 900);
		test.mergomatic(null, "table", new Text("00004"), null, 1000, false);
		assertEquals(1, merges.size());
		assertArrayEquals(new int[] { 4, 5, 6, 900 }, sizes(merges.get(i = 0)));
		merges = new ArrayList<>();
		// Limit the upper end of the merges
		test = mockMerge(merges, 1, 2, 1000, 17, 1000, 4, 5, 6, 900);
		test.mergomatic(null, "table", null, new Text("00004"), 1000, false);
		assertEquals(1, merges.size());
		assertArrayEquals(new int[] { 1, 2 }, sizes(merges.get(i = 0)));
		merges = new ArrayList<>();
		// Limit both ends
		test = mockMerge(merges, 1, 2, 1000, 17, 1000, 4, 5, 6, 900);
		test.mergomatic(null, "table", new Text("00002"), new Text("00004"), 1000, true);
		assertEquals(1, merges.size());
		assertArrayEquals(new int[] { 17, 1000 }, sizes(merges.get(i = 0)));
		merges = new ArrayList<>();
		// Clump up tablets into larger values
		test = mockMerge(merges, 100, 250, 500, 600, 100, 200, 500, 200);
		test.mergomatic(null, "table", null, null, 1000, false);
		assertEquals(3, merges.size());
		assertArrayEquals(new int[] { 100, 250, 500 }, sizes(merges.get(i = 0)));
		assertArrayEquals(new int[] { 600, 100, 200 }, sizes(merges.get(++i)));
		assertArrayEquals(new int[] { 500, 200 }, sizes(merges.get(++i)));
	}
