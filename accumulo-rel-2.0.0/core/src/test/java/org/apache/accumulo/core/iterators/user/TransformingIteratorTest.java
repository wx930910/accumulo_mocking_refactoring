...
public class TransformingIteratorTest {
	...
	public void testIdentityScan() throws Exception {
		...
		// This is just an identity scan, but with the "reuse" iterator that
		// reuses
		// the same key/value pair for every getTopKey/getTopValue call. The
		// code
	...
	public void testVisbilityFiltering() throws Exception {
		// Should return nothing since we produced visibilities that can't be
		// seen
	...
	public void testCreatingIllegalVisbility() throws Exception {
		// illegal visibility created by transform should be filtered on scan,
		// even if evaluation is
		// done
	...
	public void testRangeStart() throws Exception {
		...
		TreeMap<Key, Value> expected = new TreeMap<>();
		putExpected(expected, 1, 2, 2, 1, PartialKey.ROW_COLFAM_COLQUAL); // before
																			// the
																			// range
																			// start,
																			// but
	...
	public void testCompactionDoesntFilterVisibilities() throws Exception {
		// In scan mode, this should return nothing since it produces
		// visibilities
	...
	public static class BadVisCompactionKeyTransformingIterator extends BadVisKeyTransformingIterator {
		...
		public void init(SortedKeyValueIterator<Key, Value> source, Map<String, String> options,
				IteratorEnvironment env) throws IOException {
			env = Mockito.mock(IteratorEnvironment.class);
			Mockito.when(env.getIteratorScope()).thenReturn(IteratorScope.majc);
