 ...
 * they're supposed to and do what they should, and that cached blocks are
...
public class TestLruBlockCache {
	...
	public void testConfiguration() {
		...
		String defaultPrefix = BlockCacheManager.CACHE_PROPERTY_BASE + LruBlockCacheConfiguration.PROPERTY_PREFIX
				+ ".default.";
	...
	public void testCacheEvictionTwoPriorities() throws Exception {
		...
		LruBlockCacheConfiguration.builder(CacheType.INDEX).useEvictionThread(false).minFactor(0.98f)
				.acceptableFactor(0.99f).singleFactor(0.25f).multiFactor(0.50f).memoryFactor(0.25f).buildMap()
				.forEach(cc::set);
		manager.start(new BlockCacheConfiguration(cc));
		LruBlockCache cache = (LruBlockCache) manager.getBlockCache(CacheType.INDEX);
	...
	public void testCacheEvictionThreePriorities() throws Exception {
		...
		cc.set(Property.TSERV_INDEXCACHE_SIZE, Long.toString(maxSize));
		LruBlockCacheConfiguration.builder(CacheType.INDEX).useEvictionThread(false).minFactor(0.98f)
				.acceptableFactor(0.99f).singleFactor(0.33f).multiFactor(0.33f).memoryFactor(0.34f).buildMap()
				.forEach(cc::set);
		manager.start(new BlockCacheConfiguration(cc));
		LruBlockCache cache = (LruBlockCache) manager.getBlockCache(CacheType.INDEX);
	...
	private long calculateBlockSizeDefault(long maxSize, int numBlocks) {
		...
		return ClassSize.align((long) Math
				.floor((roughBlockSize - negateBlockSize) * LruBlockCacheConfiguration.DEFAULT_ACCEPTABLE_FACTOR));
	}
